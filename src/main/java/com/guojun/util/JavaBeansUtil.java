...
public class JavaBeansUtil {
    ...
    private static Logger logger = LoggerFactory.getLogger(JavaBeansUtil.class);
    ...
    public static String jdbcType2JavaType(String jdbcType) {
        ...
        for (Object key : properties.keySet()) {
            ...
            if (value.indexOf(jdbcType.toLowerCase()) > -1) {
                return key.toString();
            }
        }
        return "";
    ...
    private static void initTypeProperties() {
        InputStream is = null;
        try {
            is = AppProperties.class.getResourceAsStream("/typemapping.properties");
        } catch (Throwable t) {
            logger.debug("initTypeMap load happend error");
        }
        if (is != null) {
            try {
